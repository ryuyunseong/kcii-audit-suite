# 기본 제품 개발 완료 및 현재 유지보수 기준

`kcii-audit-suite v1.4.0`은 오프라인 취약점 분석·평가 보조 흐름의 baseline product completion 기준입니다.

현재 공개 정식 릴리스는 `v1.4.1`이며, 한글 리소스와 질의서 호환성 보강을 포함한 patch release입니다. `v1.4.0`의 최초 완료 기준과 공개 이력은 그대로 보존합니다.
...
- 패키지 버전: `1.4.1`
- 전체 테스트: `189 passed`
- 최초 완료 기준: `v1.4.0` (`178369b`)
- 배포: 공개 GitHub Release
- 저장소 공개 목적: 채용 포트폴리오 검토
- PyPI/TestPyPI: 미배포

`v1.4.1`과 이전 태그 및 Release asset은 이동하거나 교체하지 않습니다. 이후 수정은 새 patch 또는 minor 버전으로 분리합니다.
...
- `dev/v1.4.2`: 문서 수정과 좁은 범위의 버그 수정
- `dev/v1.5.0`: 하위 호환 신규 기능
- `release/public-readiness`: 공개 정책과 포트폴리오 검토 기록

현재 작업 브랜치의 변경은 고정된 `v1.4.1`과 이전 태그 및 asset에 반영하지 않습니다.
