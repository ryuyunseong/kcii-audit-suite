-- MySQL read-only KCII DBMS evidence summary.
...
SELECT 'schema_version=kcii-mysql-paste-v1' AS kcii_evidence;
SELECT 'dbms=mysql' AS kcii_evidence;
...
SELECT CONCAT(
  ...
  IF(EXISTS (
    ...
    WHERE User = 'root' AND account_locked = 'N'
  ), 'true', 'false')
) AS kcii_evidence;
...
SELECT CONCAT(
  ...
  IF((
    SELECT COUNT(*)
    FROM mysql.user
    WHERE Super_priv = 'Y' AND account_locked = 'N'
  ) > 1, 'true', 'false')
) AS kcii_evidence;
...
SELECT CONCAT(
  ...
  IF((
    SELECT COUNT(*)
    FROM mysql.user
    WHERE plugin NOT IN ('caching_sha2_password', 'auth_socket')
  ) = 0, 'true', 'false')
) AS kcii_evidence;
...
SELECT CONCAT(
  ...
  IF(EXISTS (
    ...
    WHERE variable_name IN ('audit_log_policy', 'audit_log_format')
  ), 'true', 'false')
) AS kcii_evidence;
