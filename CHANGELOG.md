...
- CLI 도움말과 주요 오류 메시지를 한글화했습니다.
- README, 프로파일 지원 범위, 완료 기준, 유지보수 정책과 공개 점검 문서를 현재 public 포트폴리오 상태에 맞게 정리했습니다.
- 보안장비 질의서의 시트 내용과 입력 헤더를 한글화하고 기존 영어 헤더 import 호환성을 유지했습니다.
- Unix 및 보안장비 rulepack의 손상된 한글 판정 근거와 권고 문구를 복구했습니다.

### Tests

- Runtime rulepack과 질의서 resource에 `???` 손상 문자열이 남지 않는지 검증하는 회귀 테스트를 추가했습니다.
- 한글 질의서 export/import와 Unix·보안장비 7개 산출물 smoke를 검증했습니다.
